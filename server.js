...
function buildGoldenDatasetGuidance(context = {}) {
  ...
  return [
    ...
    "AI는 이 리포트의 문체, 문단 구조, 표현 방식, 현실 조언 방식, 결론 방식만 참고한다.",
    "Golden Report 안의 고객 고유 내용, 개인정보, 생년월일, 사건, 직업, 관계 사정, 구체 문장은 복사하지 않는다.",
    "목표는 고객 내용을 재사용하는 것이 아니라 현명역학원 스타일을 재현하는 것이다.",
    "golden_dataset은 review_dataset, RAG, Knowledge Rule보다 우선하는 AI 스타일 최우선 기준이다.",
    "",
    "[Golden Report - style reference only]",
...
function buildDeveloperDebugPayload({ report, prompt, ragContext, sources, gptResponse, finalMarkdown, goldenDatasetGuidance }) {
  return {
      ...
      if (developerPreview) {
        response.developer_debug = buildDeveloperDebugPayload({
          ...
          prompt: [
            ...
            "",
            "우선순위:",
            "1. golden_dataset: 현재 요청과 가장 유사한 검수 완료 리포트의 스타일 기준",
            "2. review_dataset: 현명역학원이 승인한 수정 기록",
            "3. RAG: 계산/검색/참고 문맥",
            "4. Knowledge Rule: 기본 작성 규칙과 세계관",
            "",
            "현명역학원 Golden Dataset 검색 결과 - 스타일 전용 참고:",
      ...
      messages: [
        {
          role: "system",
          content:
            "너는 명리학 기반 사주 리포트를 사주전쟁 세계관으로 해석하는 프리미엄 리포트 작가다. 우선순위는 golden_dataset, review_dataset, RAG, Knowledge Rule 순서다. 답변 생성 전에 현재 요청과 가장 유사한 Golden Report를 스타일 기준으로 먼저 참고한다. 단 Golden Report의 고객 고유 내용, 개인정보, 사건, 직업, 관계 사정, 구체 문장은 복사하지 말고 문체, 문단 구조, 표현 방식, 현실 조언 방식, 결론 방식만 재현한다. 그 다음 승인된 Review Dataset의 수정 기준을 따른다. 운명을 겁주지 말고, 원국의 구조와 지장간 아이템, 대운 스위치, 세운 타이밍을 행동 전략으로 연결한다.",
        },
        {
          role: "user",
          content: prompt,
