...
function appendReviewRecord(payload) {
  ...
  const record = {
    ...
    after: payload.after || "",
    reason: reasons,
    reviewer: payload.reviewer || "SAJUWAR",
...
function buildReviewDatasetGuidance() {
  ...
  const examples = approvedItems
    ...
    .join("\n\n");
  return [
    "아래는 SAJUWAR가 직접 수정하고 승인한 문장 데이터다.",
...
function buildGoldenDatasetGuidance(context = {}) {
  ...
  return [
    ...
    "이 Golden Report는 SAJUWAR가 검수 완료한 최고 품질 정답 데이터다.",
    "생성 순서는 Decision Rule, Consultation Strategy, Golden Example, 고객 정보 순서다.",
    "AI는 brain.decision_rule과 brain.consultation_strategy를 먼저 사고 규칙으로 삼고, 그 다음 Golden Example의 문체, 문단 구조, 표현 방식, 현실 조언 방식, 결론 방식을 내부 기준으로 활용한다.",
    "Golden Report 안의 고객 고유 내용, 개인정보, 생년월일, 사건, 직업, 관계 사정, 구체 문장은 복사하지 않는다.",
    "목표는 사례를 재사용하는 것이 아니라 SAJUWAR의 판단 순서로 설명하는 것이다.",
...
function makeSewoon(startYear = new Date().getFullYear()) {
  ...
  return {
    startInfo: `${startYear}년부터 각 연도의 세운을 순서대로 정리합니다. 10년 단위의 큰 환경 변화는 대운이고, 세운은 그 안에서 실제로 아이템을 꺼내 쓰는 해마다의 타이밍입니다.`,
    list,
  };
}
...
현실에서는 어떻게 나타나는가: 10년 단위의 큰 흐름은 대운이 보여주고, 세운은 그 대운 안에서 해마다 달라지는 사건과 실행 타이밍을 보여줍니다. ${sewoon.startInfo} 어떤 해에는 준비가 맞고, 어떤 해에는 제안과 실행이 맞습니다. 중요한 건 좋은 시기를 기다리는 게 아니라, 시기마다 해야 할 행동을 다르게 가져가는 거예요.
      ...
      if (developerPreview) {
        response.developer_debug = buildDeveloperDebugPayload({
          ...
          prompt: [
            ...
            "2. review_dataset: SAJUWAR가 승인한 수정 기록",
            "3. RAG: 계산/검색 문맥",
            "4. Knowledge Rule: 기본 작성 규칙과 세계관",
            "",
            "SAJUWAR Golden Dataset 검색 결과 - Brain 우선 활용:",
            goldenDatasetGuidance || "등록된 golden_dataset 없음",
            "",
            "SAJUWAR Review Dataset 우선 활용:",
...
- 대운은 약 10년 단위의 큰 환경과 인생 배경이고, 세운은 그 대운 안에서 해마다 작동하는 흐름이다. "세운이 10년 흐름을 보여준다"라고 쓰지 않는다.
...
- "현명역학원", 외부 역학원·상담소 이름, "원문 관법", "자료에 따르면" 같은 출처 표현은 고객 리포트에 절대 출력하지 않는다. 모든 해설은 SAJUWAR의 직접 분석 문장으로 작성한다.
...
`;
    ...
    prompt += [
      ...
      "Use SAJUWAR style: saju evidence first, philosophical meaning second, realistic application third, caution fourth, action fifth.",
        ...
        messages: [
          {
            role: "system",
            content:
              "너는 SAJUWAR 명리 논리로 사주 리포트를 작성하는 프리미엄 상담가다. 해석은 반드시 음양, 오행, 천간, 지지, 지장간, 십성적 관계, 계절감, 월지의 힘, 대운, 세운의 순서로 판단한다. 사주 구조에서 출발하지 않는 자기계발식 예시나 뜬금없는 사업/계약/브랜드 예시는 금지한다. 우선순위는 golden_dataset, review_dataset, RAG, Knowledge Rule 순서다. 답변 생성 전에 현재 요청과 가장 유사한 Golden Report를 찾고, Decision Rule, Consultation Strategy, Golden Example, 고객 정보 순서로 사고한다. 단 Golden Report의 고객 고유 내용, 개인정보, 사건, 직업, 관계 사정, 구체 문장은 복사하지 말고 판단 규칙, 설명 순서, 문체, 문단 구조, 현실 조언 방식, 결론 방식만 재현한다. 현명역학원이나 다른 역학원·상담소 이름, 원문 관법, 자료 출처를 고객에게 절대 언급하지 말고 모든 해설은 SAJUWAR의 직접 분석으로 작성한다. 운명을 겁주지 말고, 원국의 구조와 지장간, 대운, 세운을 철학적 판단과 현실 조절 전략으로 연결한다.",
          },
          {
            role: "system",
            content:
