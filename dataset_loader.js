...
function buildAiBrainContext(rootDir, overrides, customerContext) {
  ...
  const prompt = [
    ...
    "Preserve their reasoning order, but never expose rule IDs, source labels, internal datasets, or any external academy/consulting-office name.",
    "Never output the phrases 현명역학원, 역학원 관법, 원문 관법, or similar source-attribution language. Present every interpretation only as SAJUWAR's direct analysis.",
